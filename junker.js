...
table.push([{ colSpan: 3, content: chalk.blue.bold('Info') }], [{ colSpan: 3, content: chalk.keyword('grey')(' tcup') }], [chalk.white.bold('  date:'), '\t' + today, '\t'], [chalk.white.bold('  client:'), '\t0.0.35', '\t'], [chalk.white.bold('  api:'), '\t0.0.1', '\t'], ['\t', '\t', '\t'] // needs to be /t otherwise throws off formatting
);
table.push([{ colSpan: 3, content: chalk.bold.keyword('orange')('DynamoDB') }], [{ colSpan: 3, content: chalk.keyword('grey')(' tncdb38b2e622') }], [chalk.white.bold('  rcu:'), '\t$0.00013 per hour for units of read capacity beyond the free tier', chalk.green.bold('\t$166.5')], [chalk.grey('   units:'), chalk.grey('\t1000 REQ'), '\t'], [chalk.white.bold('  wcu:'), '\t$0.00065 per hour for units of write capacity beyond the free tier', chalk.green.bold('\t$13')], [chalk.grey('   units:'), chalk.grey('\t1000 REQ'), '\t'], [chalk.white.bold('  storage:'), '\t$0.25 per GB-Month of storage used beyond first 25 free GB-Months', chalk.green.bold('\t$2.6')], [chalk.grey('   units:'), chalk.grey('\t50 GB'), '\t']);
table.push([{ colSpan: 3, content: chalk.bold.keyword('orange')('Cognito') }], [{ colSpan: 3, content: chalk.keyword('grey')(' tncup') }], [chalk.white.bold('  pool:'), '\tCognito User Pools us-east-1 tier 1 pricing', chalk.green.bold('\t$40')], [chalk.grey('   units:'), chalk.grey('\t5000 USER'), '\t']);
table.push(['\t', { hAlign: 'right', content: chalk.green.bold('TOTAL (USD)') }, '\t'], // Note: the /t in content
...
table.push(['\t', { hAlign: 'right', content: chalk.green.bold('TOTAL (AUD)') }, '\t'], // Note: the /t in content
...
['\t', { hAlign: 'right', content: chalk.white.bold('month:') }, chalk.green.bold('\t$1200 AUD')]);
// terminal stdout
...
['\t', { hAlign: 'right', content: 'month:' }, '\t$1200 AUD']);
...
// if -o-markdown
// Reset the table, plain text need to remove the colors
var file = 'Appsync_Cognito_Waf.tcup.md'; // get file name, added .tcup.txt
var newFile = fs.createWriteStream(file, { flags: 'w' }); // we want to wipe it each time
